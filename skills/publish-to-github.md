...
**对于 Skill 项目，必须包含详细的 README.md 文件**

README 文件应包含以下内容：
1. **项目标题** - 使用一级标题 (#) + 项目名称
2. **功能介绍** - 简短描述项目功能（可以使用 emoji）
3. **安装步骤** - 如何使用/安装此项目
4. **使用方法** - 具体的使用示例和命令
5. **功能说明** - 详细的功能列表或说明
6. **常见问题** - FAQ 和解决方案
7. **文件说明** - 项目中各文件的作用（可以使用表格）
8. **许可证信息** - 引用 LICENSE 文件
...
检查是否存在 README.md，如不存在或内容不完整，需要创建或更新。
...
**询问用户目标仓库地址**，格式如：
- `git@github.com:quzp1993/仓库名.git` (SSH方式)
- `https://github.com/用户名/仓库名.git` (HTTPS方式)
...
2. **已有仓库**：确认远程仓库地址
3. **README 文件**：**对于 Skill 项目，必须确认 README.md 存在且内容完整**
4. **隐私文件**：提醒用户检查待提交文件，避免泄露敏感信息
5. **选择连接方式**：推荐SSH方式（免密），HTTPS方式（需输入密码）
